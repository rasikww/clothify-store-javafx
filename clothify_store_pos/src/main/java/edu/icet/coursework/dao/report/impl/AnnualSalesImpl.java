...
public class AnnualSalesImpl implements ReportBehaviour {
    @Override
    public boolean generate(Report report) {
        ...
        try {
            Integer selectedYear = (Integer) report.getOther();//getOther contains selectedYear INT
            ...
            String sqlTotalCustomers = "SELECT COUNT(DISTINCT customer_id) FROM _order " +
                    "WHERE YEAR(order_datetime) = :selectedYear";
            ...
            JasperDesign jasperDesign = JRXmlLoader.load("src/main/resources/reports/annual_sales.jrxml");
